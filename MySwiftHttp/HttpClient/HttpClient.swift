...
class HttpClient: NSObject,URLSessionDelegate {
	...
	func httpRequest(with path:String,requestType type:RequestType,paramOfType paramType:ParamType,parameter param:Any?) -> Promise<(data: Data, response: URLResponse)> {
		...
		return session.dataTask(.promise, with: request).validate()
    }
